...
export default function AdmissionForm() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 py-4 sm:py-8 px-2 sm:px-4 md:px-8 relative overflow-x-hidden">
      {/* Background decorations - adjusted for mobile */}
      <div className="absolute top-0 left-0 w-full h-[400px] sm:h-[600px] md:h-[800px] bg-gradient-to-b from-[#2E0249] via-[#4F0187] to-transparent pointer-events-none"></div>
      <div className="absolute top-[150px] sm:top-[200px] md:top-[300px] right-[-100px] sm:right-[-150px] w-[300px] sm:w-[400px] md:w-[500px] h-[300px] sm:h-[400px] md:h-[500px] bg-blue-500/10 rounded-full blur-[80px] sm:blur-[100px] animate-pulse delay-1000 pointer-events-none"></div>
      <div className="absolute bottom-[-50px] sm:bottom-[-100px] left-1/3 w-[250px] sm:w-[300px] md:w-[400px] h-[250px] sm:h-[300px] md:h-[400px] bg-purple-500/10 rounded-full blur-[60px] sm:blur-[80px] animate-pulse delay-500 pointer-events-none"></div>

      {/* Floating Elements - hidden on very small screens */}
      <div className="hidden sm:block absolute top-20 left-10 animate-float pointer-events-none">
        <div className="w-6 h-6 sm:w-8 sm:h-8 bg-purple-500/20 rounded-full"></div>
      </div>
      <div className="hidden sm:block absolute top-40 right-20 animate-float delay-1000 pointer-events-none">
        <div className="w-5 h-5 sm:w-6 sm:h-6 bg-pink-500/20 rounded-full"></div>
      </div>
      ...
      <div className="w-full max-w-6xl mx-auto relative">
        ...
        <div className="relative w-full max-w-6xl mx-auto mb-8 sm:mb-12 group perspective-1000 px-2 sm:px-4">
          {/* Animated Glow Background */}
          <div className="absolute -inset-1 bg-gradient-to-r from-fuchsia-600 via-purple-600 to-indigo-600 rounded-2xl sm:rounded-[2.5rem] md:rounded-[3.5rem] blur-xl sm:blur-2xl opacity-40 group-hover:opacity-70 transition duration-1000 group-hover:duration-300 animate-gradient-xy"></div>

          {/* Main Glassmorphism Card */}
          <div className="relative bg-white/85 backdrop-blur-2xl border border-white/60 shadow-2xl rounded-2xl sm:rounded-[1.5rem] md:rounded-[1rem] overflow-hidden px-3 sm:px-6 md:px-8 py-6 sm:py-10 text-center transform transition-all duration-500 hover:scale-[1.01] sm:hover:scale-[1.02]">
            {/* Top Accent Line */}
            <div className="absolute top-0 left-0 w-full h-1.5 sm:h-2.5 bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500"></div>

            {/* Corner Decorative Blurs - hidden on small screens */}
            <div className="hidden sm:block absolute top-0 right-0 w-32 sm:w-48 h-32 sm:h-48 bg-purple-400/20 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"></div>
            <div className="hidden sm:block absolute bottom-0 left-0 w-32 sm:w-48 h-32 sm:h-48 bg-indigo-400/20 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2"></div>

            {/* Floating Icons - hidden on small screens */}
            <div className="hidden lg:block absolute top-12 left-6 md:left-12 bg-white/60 backdrop-blur-md p-3 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-white animate-float-slow">
              <BookOpen size={28} strokeWidth={1.5} className="text-indigo-600" />
            </div>
            <div className="hidden lg:block absolute bottom-16 right-6 md:right-12 bg-white/60 backdrop-blur-md p-4 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-white animate-float-fast">
              <GraduationCap size={32} strokeWidth={1.5} className="text-purple-600" />
            </div>
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
              ...
              <div className="mb-6 sm:mb-10 relative">
                <div className="absolute inset-0 bg-purple-200 rounded-full blur-xl sm:blur-2xl opacity-50 animate-pulse"></div>
                <div className="relative bg-white p-2 sm:p-4 rounded-2xl sm:rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.08)] border border-gray-50 transform group-hover:-translate-y-1 sm:group-hover:-translate-y-2 transition-transform duration-500">
                  <Image
                    src={"/img/logo.png"}
                    alt="School Logo"
                    height={90}
                    width={240}
                    className="w-auto h-[40px] sm:h-[60px] md:h-[90px] object-contain drop-shadow-sm"
                    priority
                  />
                </div>
              </div>
              ...
              <h1 className="text-3xl sm:text-5xl md:text-7xl font-black mb-2 sm:mb-3">
                <span className="bg-gradient-to-r from-purple-900 via-purple-700 to-indigo-800 bg-clip-text text-transparent drop-shadow-sm px-2 sm:px-10">
                  ভর্তি ফর্ম
                </span>
              </h1>

              {/* Subtitle / Badge */}
              <div className="inline-flex items-center justify-center gap-1 sm:gap-2 mb-6 sm:mb-8 bg-purple-50/80 backdrop-blur-sm px-3 sm:px-6 py-1.5 sm:py-2.5 rounded-full border border-purple-200/60 shadow-inner">
                <BookOpen size={14} className="text-purple-500 animate-pulse" />
                <p className="text-purple-800 font-bold text-xs sm:text-base md:text-lg">
                  আগামীর আলোকিত ভবিষ্যতের সন্ধানে
                </p>
                <GraduationCap
                  size={14}
              ...
              {/* Animated Separator Dots & Lines */}
              <div className="flex items-center justify-center gap-2 sm:gap-3 md:gap-4 mt-2">
                <div className="w-10 sm:w-16 md:w-32 h-0.5 sm:h-1 bg-gradient-to-r from-transparent to-purple-500 rounded-full opacity-50"></div>
                <div className="flex gap-1.5 sm:gap-2.5">
                  <div className="w-2 h-2 sm:w-3 sm:h-3 bg-purple-500 rounded-full animate-bounce [animation-delay:-0.3s] shadow-[0_0_12px_rgba(168,85,247,0.6)]"></div>
                  <div className="w-2 h-2 sm:w-3 sm:h-3 bg-indigo-500 rounded-full animate-bounce [animation-delay:-0.15s] shadow-[0_0_12px_rgba(99,102,241,0.6)]"></div>
                  <div className="w-2 h-2 sm:w-3 sm:h-3 bg-fuchsia-500 rounded-full animate-bounce shadow-[0_0_12px_rgba(236,72,153,0.6)]"></div>
                </div>
                <div className="w-10 sm:w-16 md:w-32 h-0.5 sm:h-1 bg-gradient-to-l from-transparent to-indigo-500 rounded-full opacity-50"></div>
              </div>
            </div>
          </div>
        </div>
          ...
          <Card className="border-2 border-purple-100/50 shadow-xl overflow-hidden backdrop-blur-sm bg-white/90 rounded-xl sm:rounded-2xl">
            ...
            <CardFooter className="bg-gray-50/50 border-t border-gray-200 p-4 sm:p-6 flex flex-col sm:flex-row justify-between gap-3 sm:gap-0">
              <Button
                ...
                onClick={handleBack}
                disabled={currentStep === 1}
                className="w-full sm:w-auto px-4 sm:px-6 py-2 rounded-xl border-2 hover:bg-purple-50 hover:border-purple-300 transition-all order-2 sm:order-1"
              >
                <ArrowLeft size={16} className="mr-2" />
                পেছনে
              </Button>
